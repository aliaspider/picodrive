...
void menu_draw_selection(int x, int y, int w)
{
   ...
   text_out16_(x, y, (void*)1,
               (menu_sel_color < 0) ? menu_text_color : menu_sel_color);
...
void menu_init(void)
{
   ...
   for (c = 0; c < 256; c++)
   {
      for (l = 0; l < 8; l++)
      {
         ...
         if (fd8x8 & 0x40) *fd |= 0x0f;
         fd++;
         if (fd8x8 & 0x20) *fd |= 0xf0;
         if (fd8x8 & 0x10) *fd |= 0x0f;
         fd++;
         if (fd8x8 & 0x08) *fd |= 0xf0;
         if (fd8x8 & 0x04) *fd |= 0x0f;
         fd++;
         if (fd8x8 & 0x02) *fd |= 0xf0;
         if (fd8x8 & 0x01) *fd |= 0x0f;
         fd++;
      }
      fd += 8 * 2 / 2; // 2 empty lines
   }

   memcpy(menu_font_data, menu_font_data + ((int)'>') * 4 * 10,
          4 * 10); // default selector symbol is '>'
...
void me_draw(const menu_entry* entries, int count, int x, int y,
             me_draw_custom_f* cust_draw, void* param)
{
   int i, y1 = y;
   ...
   for (i = 0; i < count; i++)
   {
      ...
      if (entries[i].beh == MB_ONOFF)
         text_out16(x + 27 * 8, y1,
                    (*(int*)entries[i].var & entries[i].mask) ? "ON" : "OFF");
