...
static void custom_write(FILE* f, const menu_entry* me, int no_def)
{
   ...
   case MA_OPT_REGION:
      ...
      strncpy(str24, me_region_name(PicoRegionOverride, PicoAutoRgnOrder), 23);
      str24[23] = 0;
      fprintf(f, "Region = %s", mystrip(str24));
      break;
   case MA_OPT_CONFIRM_STATES:
      if (no_def
            && !((defaultConfig.EmuOpt ^ currentConfig.EmuOpt) & (5 << 9))) return;
   ...
   case MA_OPT3_VSYNC:
      if (no_def
            && (defaultConfig.EmuOpt & 0x12000) == (currentConfig.gamma2 & 0x12000)) return;
      strcpy(str24, "never");
...
static void keys_write(FILE* fn, const char* bind_str, int dev_id,
                       const int* binds, int no_defaults)
{
   char act[48];
   int key_count, t, i;
   const int* def_binds;
...
static int default_var(const menu_entry* me)
{
   ...
   case MA_OPT_SCALING:
      return defaultConfig.scaling;
   case MA_OPT_ROTATION:
      return defaultConfig.rotation;
...
int config_writesect(const char* fname, const char* section)
{
   ...
   if (section != NULL)
   {
      ...
      while (!feof(fo))
      {
         ...
         if (line[0] == '[' && strncmp(line + 1, section, tlen) == 0
               && line[tlen + 1] == ']')
            break;
         fputs(line, fn);
      }
 ...
 */
int config_get_var_val(void* file, char* line, int lsize, char** rvar,
                       char** rval)
{
   char* var, *val, *tmp;
   FILE* f = file;
