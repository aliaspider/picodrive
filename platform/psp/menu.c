...
unsigned long wait_for_input(unsigned int interesting, int is_key_config)
{
   ...
   for (count = 0; !((ret = psp_pad_read(1)) & interesting)
         && count < 100; count++)
   {
      psp_msleep(50);
      release = 1;
   ...
   if (wait > 6
         && (ret & (PBTN_UP | PBTN_LEFT | PBTN_DOWN | PBTN_RIGHT | PBTN_L | PBTN_R)))
      wait = 6;
...
void menu_draw_begin(void)
{
   ...
   sceGuCopyImage(GU_PSM_5650, 0, 0, 480, 272, 480, bg_buffer, 0, 0, 512,
                  menu_screen);
   sceGuFinish();
   sceGuSync(0, 0);
}
...
static void draw_dirlist(char* curdir, struct my_dirent** namelist, int n,
                         int sel)
{
   int start, i, pos;
   ...
   menu_darken_bg((char*)menu_screen + 512 * 129 * 2,
                  (char*)menu_screen + 512 * 129 * 2, 512 * 10, 0);
                      ...
                      int(*compar)(const void*, const void*))
{
   ...
   // sort
   if (compar != NULL
         && name_count > 3) qsort(&namelist[2], name_count - 2, sizeof(namelist[0]),
                                     compar);
...
static char* romsel_loop(char* curr_path)
{
   ...
   for (;;)
   {
      ...
      inp = wait_for_input(PBTN_UP | PBTN_DOWN | PBTN_LEFT | PBTN_RIGHT | PBTN_L |
                           PBTN_R | PBTN_X | PBTN_CIRCLE, 0);
      if (inp & PBTN_UP)
      {
         sel--;
         if (sel < 0)   sel = n - 2;
...
static void draw_patchlist(int sel)
{
   ...
   for (i = 0; i < PicoPatchCount; i++)
   {
      ...
      smalltext_out16_lim(14,     pos * 10, active ? "ON " : "OFF",
                          active ? 0xfff6 : 0xffff, 3);
      smalltext_out16_lim(14 + 6 * 4, pos * 10, PicoPatches[i].name,
                          active ? 0xfff6 : 0xffff, 53 - 6);
   }
   pos = start + i;
...
static void patches_menu_loop(void)
{
   ...
   for (;;)
   {
      ...
      {
         menu_sel--;
         if (menu_sel < 0) menu_sel = PicoPatchCount;
      }
      if (inp & PBTN_DOWN)
      {
         menu_sel++;
         if (menu_sel > PicoPatchCount) menu_sel = 0;
      }
      if (inp & (PBTN_LEFT | PBTN_L))
      {
         menu_sel -= 10;
         if (menu_sel < 0) menu_sel = 0;
      }
      if (inp & (PBTN_RIGHT | PBTN_R))
      {
         menu_sel += 10;
         if (menu_sel > PicoPatchCount) menu_sel = PicoPatchCount;
      }
...
{
   unsigned char* ptr = malloc(sizeof(Pico.vram) + sizeof(Pico.cram) + sizeof(
                                  Pico.vsram) + sizeof(Pico.video));
   if (ptr == NULL) return NULL;

   memcpy(ptr, Pico.vram, sizeof(Pico.vram));
   memcpy(ptr + sizeof(Pico.vram), Pico.cram, sizeof(Pico.cram));
   memcpy(ptr + sizeof(Pico.vram) + sizeof(Pico.cram), Pico.vsram,
          sizeof(Pico.vsram));
   memcpy(ptr + sizeof(Pico.vram) + sizeof(Pico.cram) + sizeof(Pico.vsram),
          &Pico.video, sizeof(Pico.video));
   return ptr;
}
...
static void restore_oldstate(void* ptrx)
{
   ...
   memcpy(Pico.vsram, ptr + sizeof(Pico.vram) + sizeof(Pico.cram),
          sizeof(Pico.vsram));
   memcpy(&Pico.video, ptr + sizeof(Pico.vram) + sizeof(Pico.cram) + sizeof(
             Pico.vsram), sizeof(Pico.video));
   free(ptrx);
}
...
static void draw_savestate_menu(int menu_sel, int is_loading)
{
   ...
   y = tl_y;
   for (i = 0; i < 10; i++, y += 10)
      text_out16(tl_x, y, "SLOT %i (%s)", i,
                 (state_slot_flags & (1 << i)) ? "USED" : "free");
...
static int savestate_menu_loop(int is_loading)
{
   ...
   for (;;)
   {
      ...
      if (inp & PBTN_UP)
      {
         ...
         {
            menu_sel--;
            if (menu_sel < 0) menu_sel = menu_sel_max;
         }
         while (!(state_slot_flags & (1 << menu_sel)) && menu_sel != menu_sel_max
                && is_loading);
      }
      if (inp & PBTN_DOWN)
      {
         ...
         {
            menu_sel++;
            if (menu_sel > menu_sel_max) menu_sel = 0;
         }
         while (!(state_slot_flags & (1 << menu_sel)) && menu_sel != menu_sel_max
                && is_loading);
      }
...
static char* action_binds(int player_idx, int action_mask)
{
   ...
   for (i = 0; i < 32; i++) // i is key index
   {
      ...
      {
         if (player_idx >= 0
               && ((currentConfig.KeyBinds[i] >> 16) & 3) != player_idx) continue;
         if (strkeys[0])
         {
...
static int count_bound_keys(int action, int pl_idx)
{
   ...
   {
      if (pl_idx >= 0
            && (currentConfig.KeyBinds[i] & 0x30000) != (pl_idx << 16)) continue;
...
static void draw_key_config(const me_bind_action* opts, int opt_cnt,
                            int player_idx, int sel)
{
   ...
   for (i = 0; i < opt_cnt; i++, y += 10)
      text_out16(x, y, "%s : %s", opts[i].name, action_binds(player_idx,
                 opts[i].mask));
...
static void key_config_loop(const me_bind_action* opts, int opt_cnt,
                            int player_idx)
{
   ...
   for (;;)
   {
      ...
      if (!(inp & PBTN_SELECT))
      {
         ...
         {
            sel--;
            if (sel < 0) sel = menu_sel_max;
            continue;
         }
         if (inp & PBTN_DOWN)
         {
            sel++;
            if (sel > menu_sel_max) sel = 0;
            continue;
         }
      }
      if (sel >= opt_cnt)
...
static void kc_sel_loop(void)
{
   ...
   while (1)
   {
      ...
      inp = wait_for_input(PBTN_UP | PBTN_DOWN | PBTN_LEFT | PBTN_RIGHT | PBTN_X |
                           PBTN_CIRCLE, 0);
      selected_id = me_index2id(ctrlopt_entries, CTRLOPT_ENTRY_COUNT, menu_sel);
      if (inp & (PBTN_LEFT | PBTN_RIGHT)) // multi choise
         me_process(ctrlopt_entries, CTRLOPT_ENTRY_COUNT, selected_id,
                    (inp & PBTN_RIGHT) ? 1 : 0);
      if (inp & PBTN_UP)
      {
         menu_sel--;
      ...
      if (inp & PBTN_CIRCLE)
      {
         ...
         case MA_CTRL_PLAYER1:
            key_config_loop(me_ctrl_actions, is_6button ? 15 : 11, 0);
            return;
         case MA_CTRL_PLAYER2:
            key_config_loop(me_ctrl_actions, is_6button ? 15 : 11, 1);
            return;
         case MA_CTRL_EMU:
            key_config_loop(emuctrl_actions,
                            sizeof(emuctrl_actions) / sizeof(emuctrl_actions[0]) - 1, -1);
            return;
         case MA_CTRL_DONE:
            if (!rom_loaded) emu_WriteConfig(0);
            return;
         default:
            return;
         }
...
static void menu_cdopt_cust_draw(const menu_entry* entry, int x, int y,
                                 void* param)
{
   ...
   case MA_CDOPT_TESTBIOS_USA:
      text_out16(x, y, "USA BIOS:     %s", bios_names->us);
      break;
   case MA_CDOPT_TESTBIOS_EUR:
      text_out16(x, y, "EUR BIOS:     %s", bios_names->eu);
      break;
...
static void draw_cd_menu_options(int menu_sel, struct bios_names_t* bios_names)
{
   ...
   me_draw(cdopt_entries, CDOPT_ENTRY_COUNT, tl_x, tl_y, menu_cdopt_cust_draw,
           bios_names);

   selected_id = me_index2id(cdopt_entries, CDOPT_ENTRY_COUNT, menu_sel);
   if ((selected_id == MA_CDOPT_TESTBIOS_USA
         && strcmp(bios_names->us, "NOT FOUND")) ||
...
static void cd_menu_loop_options(void)
{
   ...
   if (emu_findBios(4, &bios))   // US
   {
      ...
      if (*p == '/') p++;
      strncpy(bios_names.us, p, sizeof(bios_names.us));
      bios_names.us[sizeof(bios_names.us) - 1] = 0;
   }
   ...
   if (emu_findBios(8, &bios))   // EU
   {
      for (p = bios + strlen(bios) - 1; p > bios && *p != '/'; p--);
      if (*p == '/') p++;
      strncpy(bios_names.eu, p, sizeof(bios_names.eu));
      bios_names.eu[sizeof(bios_names.eu) - 1] = 0;
   }
   ...
   if (emu_findBios(1, &bios))   // JP
   {
      for (p = bios + strlen(bios) - 1; p > bios && *p != '/'; p--);
      if (*p == '/') p++;
      strncpy(bios_names.jp, p, sizeof(bios_names.jp));
      bios_names.jp[sizeof(bios_names.jp) - 1] = 0;
   }
   ...
   for (;;)
   {
      ...
      if (inp & (PBTN_LEFT | PBTN_RIGHT)) // multi choise
      {
         if (!me_process(cdopt_entries, CDOPT_ENTRY_COUNT, selected_id,
                         (inp & PBTN_RIGHT) ? 1 : 0) &&
...
static void menu_opt3_cust_draw(const menu_entry* entry, int x, int y,
                                void* param)
{
   switch (entry->id)
   {
   case MA_OPT3_SCALE:
      text_out16(x, y, "Scale factor:                      %.2f",
                 currentConfig.scale);
      break;
   case MA_OPT3_HSCALE32:
      text_out16(x, y, "Hor. scale (for low res. games):   %.2f",
                 currentConfig.hscale32);
      break;
   case MA_OPT3_HSCALE40:
      text_out16(x, y, "Hor. scale (for hi res. games):    %.2f",
                 currentConfig.hscale40);
      break;
   case MA_OPT3_FILTERING:
      text_out16(x, y, "Bilinear filtering                 %s",
                 currentConfig.scaling ? "ON" : "OFF");
      break;
   case MA_OPT3_GAMMAA:
...
static void dispmenu_loop_options(void)
{
   ...
   for (;;)
   {
      ...
      if (selected_id == MA_OPT3_HSCALE40 &&  is_32col)
         is_32col = 0;
      if (selected_id == MA_OPT3_HSCALE32 && !is_32col)
         is_32col = 1;
      ...
      if (inp & (PBTN_LEFT | PBTN_RIGHT)) // multi choise
      {
         float* setting = NULL;
         int tmp;
         me_process(opt3_entries, OPT3_ENTRY_COUNT, selected_id,
                    (inp & PBTN_RIGHT) ? 1 : 0);
         switch (selected_id)
         {
         case MA_OPT3_SCALE:
            setting = &currentConfig.scale;
            break;
         case MA_OPT3_HSCALE40:
            setting = &currentConfig.hscale40;
            is_32col = 0;
            break;
         case MA_OPT3_HSCALE32:
            setting = &currentConfig.hscale32;
            is_32col = 1;
            break;
         case MA_OPT3_FILTERING:
         case MA_OPT3_GAMMAA:
...
static void amenu_loop_options(void)
{
   ...
   for (;;)
   {
      ...
      if (inp & (PBTN_LEFT | PBTN_RIGHT)) // multi choise
      {
         if (!me_process(opt2_entries, OPT2_ENTRY_COUNT, selected_id,
                         (inp & PBTN_RIGHT) ? 1 : 0) &&
...
static void menu_opt_cust_draw(const menu_entry* entry, int x, int y,
                               void* param)
{
   char* str, str24[24];
   ...
   case MA_OPT_SOUND_QUALITY:
      ...
      break;
   case MA_OPT_REGION:
      text_out16(x, y, "Region:              %s", me_region_name(PicoRegionOverride,
                 PicoAutoRgnOrder));
      break;
   ...
   case MA_OPT_CPU_CLOCKS:
      text_out16(x, y, "CPU/bus clock       %3i/%3iMHz", currentConfig.CPUclock,
                 currentConfig.CPUclock / 2);
      break;
   case MA_OPT_SAVECFG:
      str24[0] = 0;
...
static int menu_loop_options(void)
{
   ...
   me_enable(opt_entries, OPT_ENTRY_COUNT, MA_OPT_LOADCFG,
             config_slot != config_slot_current);
   ...
   while (1)
   {
      ...
      if (inp & (PBTN_LEFT | PBTN_RIGHT)) // multi choise
      {
         if (!me_process(opt_entries, OPT_ENTRY_COUNT, selected_id,
                         (inp & PBTN_RIGHT) ? 1 : 0))
         {
            switch (selected_id)
            {
...
static void menu_loop_root(void)
{
   ...
   me_enable(main_entries, MAIN_ENTRY_COUNT, MA_MAIN_PATCHES,
             PicoPatches != NULL);
   ...
   {
      draw_menu_root(menu_sel);
      inp = wait_for_input(PBTN_UP | PBTN_DOWN | PBTN_X | PBTN_CIRCLE | PBTN_SELECT |
                           PBTN_L | PBTN_R, 0);
      if (inp & PBTN_UP)
      {
         menu_sel--;
      ...
      if (inp & (PBTN_SELECT | PBTN_X))
      {
         if (rom_loaded)
         {
            while (psp_pad_read(1) & (PBTN_SELECT | PBTN_X)) psp_msleep(
                  50); // wait until released
