...
static void osd_text(int x, const char* text, int is_active, int clear_all)
{
   ...
   if (is_active)
   {
      tmp = psp_screen;   // nasty pointer tricks
      psp_screen = screen;
   }
...
void emu_prepareDefaultConfig(void)
{
   ...
   defaultConfig.s_PicoOpt = 0x0f | POPT_EN_MCD_PCM | POPT_EN_MCD_CDDA |
...
static void clearArea(int full)
{
   ...
   else
   {
      ...
      memset32_uncached((int*)((char*)psp_screen + 512 * 264 * 2), 0,
                        512 * 8 * 2 / 4);
      memset32_uncached((int*)((char*)fb         + 512 * 264 * 2), 0,
                        512 * 8 * 2 / 4);
   }
}
...
static short __attribute__((aligned(4))) sndBuffer[SOUND_BLOCK_SIZE_PAL *
      SOUND_BLOCK_COUNT + 44100 / 50 * 2];
...
void emu_startSound(void)
{
   ...
   if (PsndRate != PsndRate_old || (PicoOpt & 0x0b) != (PicoOpt_old & 0x0b)
         || Pico.m.pal != pal_old)
   ...
   // sceAudio_5C37C0AE();
   ret = sceAudio_38553111(samples_block / 2, PsndRate,
                           2); // seems to not need that stupid 64byte alignment
   if (ret < 0)
   {
...
static void RunEventsPico(unsigned int events, unsigned int keys)
{
   ...
   if (pico_inp_mode != 0)
   {
      ...
      PicoPicohw.pen_pos[1] = pico_inp_mode == 1 ? (0x2f8 + pico_pen_y) :
                              (0x1fc + pico_pen_y);
   }
}
...
static void RunEvents(unsigned int which)
{
   if (which & 0x1800) // save or load (but not both)
   {
             ...
             (!(which & 0x1000) && (currentConfig.EmuOpt & 0x200))))  // save
      {
         int keys;
         sceGuSync(0, 0);
         blit2("", (which & 0x1000) ? "LOAD STATE? (X=yes, O=no)" :
               "OVERWRITE SAVE? (X=yes, O=no)", 0);
   ...
   if (which & 0x0300)
   {
      ...
      sprintf(noticeMsg, "SAVE SLOT %i [%s]", state_slot,
...
void emu_Loop(void)
{
   ...
   while (engineState == PGS_Running)
   {
      ...
      if (tval - tval_thissec >= 1000000)
      {
         ...
         if (currentConfig.Frameskip < 0)
         {
            ...
            if (pframes_done  < 0) pframes_done  = 0;
            pframes_shown -= target_fps;
            if (pframes_shown < 0) pframes_shown = 0;
      ...
      if (currentConfig.Frameskip < 0
            && tval_diff - lim_time >= (300000 << 8)) // slowdown detection
         reset_timing = 1;
   ...
   memset32_uncached((int*)psp_video_get_active_fb() + 512 * 264 * 2 / 4, 0,
                     512 * 8 * 2 / 4);
}
