...
{
   if (argc > 0)
      return pico_main(argv[1]);

   return pico_main(NULL);
...
void psp_init(void)
{
   ...
   thid = sceKernelCreateThread("update_thread", callback_thread, 0x11, 0xFA0, 0,
                                NULL);
   if (thid >= 0)
   ...
   sceDisplaySetFrameBuf(VRAM_FB1, 512, PSP_DISPLAY_PIXEL_FORMAT_565,
...
char* psp_get_status_line(void)
{
   ...
   snprintf(buff, sizeof(buff), "%02i:%02i  bat: %3i%%", time.hour, time.minutes,
            bat_percent);
   if (!scePowerIsPowerOnline())
      snprintf(buff + strlen(buff), sizeof(buff) - strlen(buff), " (%i:%02i)",
               bat_time / 60, bat_time % 60);
   return buff;
}
