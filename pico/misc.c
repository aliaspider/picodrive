...
{
   unsigned int sreg = Pico.m.sram_reg, saddr = Pico.m.eeprom_addr,
                scyc = Pico.m.eeprom_cycle, ssa = Pico.m.eeprom_slave;

   elprintf(EL_EEPROM, "eeprom: scl/sda: %i/%i -> %i/%i, newtime=%i",
            (sreg & 2) >> 1, sreg & 1,
   ...
   else if ((sreg & 8) && !(sreg & 2) && (d & 2))
   {
      ...
      else
      {
         ...
         if (scyc == 18)
         {
            ...
            if (saddr & 1)
            {
               saddr += 2;   // next addr in read mode
               saddr &= 0xff;
            }
         }
      }
   ...
   else if ((sreg & 8) && (sreg & 2) && !(d & 2))
   {
      ...
      if (SRam.eeprom_type)
      {
         ...
         else if ((SRam.eeprom_type == 3 && scyc > 27) || (SRam.eeprom_type == 2
                  && scyc > 18))
         {
            if (!(ssa & 1))
            {
               ...
               *pm |= d & 1;
               if (scyc == 26 || scyc == 35)
               {
                  saddr = (saddr & ~0xf) | ((saddr + 1) &
                                            0xf); // only 4 (?) lowest bits are incremented
                  elprintf(EL_EEPROM, "eeprom: write done, addr inc to: %x, last byte=%02x",
                           saddr, *pm);
               }
               SRam.changed = 1;
            }
         ...
         else if (scyc > 9)
         {
            if (!(ssa & 1))
            {
               ...
               saddr <<= 1;
               if (SRam.eeprom_type == 2) saddr &= 0xff;
               else saddr &= 0x1fff; // mask
               saddr |= d & 1;
               if (scyc == 17 || scyc == 26)
               {
                  elprintf(EL_EEPROM, "eeprom: addr reg done: %x", saddr);
                  if (scyc == 17 && SRam.eeprom_type == 2)
                  {
                     saddr &= 0xff;   // add device bits too
                     saddr |= (ssa << 7) & 0x700;
                  }
               }
            }
         ...
         else if (scyc > 9)
         {
            if (!(saddr & 1))
            {
               ...
               *pm |= d & 1;
               if (scyc == 17)
               {
                  saddr = (saddr & 0xf9) | ((saddr + 2) &
                                            6); // only 2 lowest bits are incremented
                  elprintf(EL_EEPROM, "eeprom: write done, addr inc to: %x, last byte=%02x",
                           saddr >> 1, *pm);
               }
               SRam.changed = 1;
...
PICO_INTERNAL_ASM unsigned int SRAMReadEEPROM(void)
{
   ...
   sreg = Pico.m.sram_reg;
   saddr = Pico.m.eeprom_addr & 0x1fff;
   scyc = Pico.m.eeprom_cycle;
   ssa = Pico.m.eeprom_slave;
   ...
   else if (scyc > 9 && scyc < 18)
   {
      ...
      if (SRam.eeprom_type)
      {
         ...
         if (ssa & 1)
         {
            elprintf(EL_EEPROM, "eeprom: read: addr %02x, cycle %i, reg %02x", saddr, scyc,
                     sreg);
            if (shift == 0) elprintf(EL_EEPROM, "eeprom: read done, byte %02x",
                                        SRam.data[saddr]);
      ...
      else
      {
         ...
         if (saddr & 1)
         {
            elprintf(EL_EEPROM, "eeprom: read: addr %02x, cycle %i, reg %02x", saddr >> 1,
                     scyc, sreg);
            if (shift == 0) elprintf(EL_EEPROM, "eeprom: read done, byte %02x",
                                        SRam.data[saddr >> 1]);
