...
static void carthw_12in1_write8(unsigned int a, unsigned int d, int realsize)
{
   ...
   if (a < 0xA13000 || a >= 0xA13040)
   {
      ...
      if (a != 0x200001)
         elprintf(EL_ANOMALY, "12-in-1: unexpected write [%06x] %02x @ %06x", a, d,
                  SekPc);
      return;
   }
...
static void carthw_realtec_write8(unsigned int a, unsigned int d, int realsize)
{
   ...
   else if (a == 0x404000)
   {
      ...
      elprintf(EL_ANOMALY, "realtec: unexpected write [%06x] %02x @ %06x", a, d,
               SekPc);

   if (realtec_bank >= 0 && realtec_size >= 0 &&
         (realtec_bank != bank_old || realtec_size != size_old))
   {
      elprintf(EL_ANOMALY, "realtec: new bank %06x, size %06x @ %06x", realtec_bank,
               realtec_size, SekPc);
      if (realtec_size > realtec_romsize - realtec_bank
            || realtec_bank >= realtec_romsize)
      {
