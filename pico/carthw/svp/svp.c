...
static void PicoSVPLine(void)
{
   ...
   delay_lines++;
   if ((Pico.m.scanline & 0xf) != 0xf && Pico.m.scanline != 261
         && Pico.m.scanline != 311)
      return;
   count = delay_lines;
   delay_lines = 0;
...
static int PicoSVPDma(unsigned int source, int len, unsigned short** srcp,
                      unsigned short** limitp)
{
   ...
   else if ((source & 0xfe0000) == 0x300000)
   {
      ...
      *limitp = (unsigned short*)(svp->dram + sizeof(svp->dram));
      ...
      elprintf(EL_VDPDMA | EL_SVP | EL_ANOMALY,
               "SVP FIXME unhandled DmaSlow from %06x, len=%i", source, len);
