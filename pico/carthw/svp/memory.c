...
unsigned int PicoSVPRead16(unsigned int a, int realsize)
{
   ...
   else if ((a & 0xfffff0) == 0xa15000)
   {
      ...
      elprintf(EL_UIO | EL_SVP | EL_ANOMALY,
               "SVP FIXME: unhandled r%i: [%06x] %04x @%06x", realsize, a & 0xffffff, d,
               SekPc);

   if (!a15004_looping)
      elprintf(EL_SVP, "SVP r%i: [%06x] %04x @%06x", realsize, a & 0xffffff, d,
               SekPc);
...
{
   elprintf(EL_UIO | EL_SVP | EL_ANOMALY, "!!! SVP w%i: [%06x], %08x @%06x",
            realsize, a & 0xffffff, d, SekPc);
}
...
void PicoSVPWrite16(unsigned int a, unsigned int d, int realsize)
{
   ...
   else if ((a & 0xfffff0) == 0xa15000)
   {
      ...
      elprintf(EL_UIO | EL_SVP | EL_ANOMALY,
               "SVP FIXME: unhandled w%i: [%06x] %04x @%06x", realsize, a & 0xffffff, d,
               SekPc);
   ...
   elprintf(EL_SVP, "SVP w%i: [%06x] %04x @%06x", realsize, a & 0xffffff, d,
            SekPc);
}
