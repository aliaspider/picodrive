...
void (*PicoCDLoadProgressCB)(int percent) =
   NULL; // handled in Pico/cd/cd_file.c
...
static unsigned char* PicoCartAlloc(int filesize)
{
   ...
   alloc_size &=
      ~0x7ffff; // use alloc size of multiples of 512K, so that memhandlers could be set up more efficiently
   if ((filesize & 0x3fff) == 0x200) alloc_size += 0x200;
   else if (alloc_size - filesize < 4) alloc_size +=
         4; // padding for out-of-bound exec protection
...
int PicoCartLoad(pm_file* f, unsigned char** prom, unsigned int* psize)
{
   ...
   if (!(PicoAHW & PAHW_MCD) && size == 0x20000
         && (!strncmp((char*)rom + 0x124, "BOOT", 4) ||
   ...
   if (size >= 0x4200 && (size & 0x3fff) == 0x200 &&
         ((rom[0x2280] == 'S' && rom[0x280] == 'E') || (rom[0x280] == 'S'
               && rom[0x2281] == 'E')))
   {
      DecodeSmd(rom, size);
      size -= 0x200; // Decode and byteswap SMD
   }
...
int PicoCartInsert(unsigned char* rom, unsigned int romsize)
{
   ...
   {
   default:
      elprintf(EL_STATUS | EL_ANOMALY, "starting in unknown hw configuration: %x",
               PicoAHW);
   case 0:
   case PAHW_SVP:
      PicoMemSetup();
...
static void PicoCartDetect(void)
{
   ...
   if (Pico.rom[0x1B1] == 'R' && Pico.rom[0x1B0] == 'A')
   {
      if (Pico.rom[0x1B2] & 0x40)
      {
         ...
         SRam.start = PicoRead32(0x1B4) &
                      ~1; // zero address is used for clock by some games
   ...
   SRam.eeprom_abits  =
      3; // eeprom access must be odd addr for: bit0 ~ cl, bit1 ~ in
            ...
            name_cmp("RINGS OF POWER") == 0)
   {
      ...
      SRam.eeprom_bit_out = 7;
            ...
            (name_cmp("        ") == 0
             && // Micro Machines {Turbo Tournament '96, Military - It's a Blast!}
