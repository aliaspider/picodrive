...
u32 z80ReadBusReq(void)
{
   ...
   elprintf(EL_BUSREQ, "get_zrun: %02x [%i] @%06x", d | 0x80, SekCyclesDone(),
            SekPc);
   return d | 0x80;
}
...
static void z80WriteBusReq(u32 d)
{
   d &= 1;
   d ^= 1;
   elprintf(EL_BUSREQ, "set_zrun: %i->%i [%i] @%06x", Pico.m.z80Run, d,
            SekCyclesDone(), SekPc);
   if (d ^ Pico.m.z80Run)
   {
      if (d)
...
static void z80WriteReset(u32 d)
{
   d &= 1;
   d ^= 1;
   elprintf(EL_BUSREQ, "set_zreset: %i->%i [%i] @%06x", Pico.m.z80_reset, d,
            SekCyclesDone(), SekPc);
...
u32 OtherRead16(u32 a, int realsize)
{
   ...
   if ((a & 0xffffe0) == 0xa10000) // I/O ports
   {
      ...
      case 0:
         d = Pico.m.hardware;
         break; // Hardware value (Version register)
      case 1:
         d = PadRead(0);
         break;
      case 2:
      ...
      default:
         d = Pico.ioports[a];
         break; // IO ports can be used as RAM
      }
      d |= d << 8;
      goto end;
   }
   ...
   if ((a & 0xff0000) == 0xa00000)
   {
      ...
      if ((a & 0x4000) == 0x0000)
      {
         d = z80Read8(a);   // Z80 ram (not byteswaped)
         d |= d << 8;
         goto end;
      }
      if ((a & 0x6000) == 0x4000)
      {
         d = ym2612_read_local_68k();   // 0x4000-0x5fff
         goto end;
      }
...
void OtherWrite8(u32 a, u32 d)
{
   ...
   if ((a & 0xe700f9) == 0xc00011 || (a & 0xff7ff9) == 0xa07f11)
   {
   ...
   if ((a & 0xff4000) == 0xa00000) // z80 RAM
   {
      ...
      else elprintf(EL_ANOMALY,
                       "68k z80 write with no bus or reset! [%06x] %02x @ %06x", a, d & 0xff, SekPc);
      return;
   }
   if ((a & 0xff6000) == 0xa04000)
   {
      if (PicoOpt & 1) emustatus |= ym2612_write_local(a & 3, d & 0xff, 0) & 1;
      // FM Sound
      return;
   }
   if ((a & 0xffffe0) == 0xa10000)
   {
      IoWrite8(a, d);   // I/O ports
      return;
   }
#endif
   if (a == 0xa11100)
...
void OtherWrite16(u32 a, u32 d)
{
   ...
   if ((a & 0xe700f8) == 0xc00010 || (a & 0xff7ff8) == 0xa07f10)
   {
      if (PicoOpt & 2) SN76496Write(d);   // PSG Sound
      return;
   }
   ...
   {
      if (!(Pico.m.z80Run & 1)
            && !Pico.m.z80_reset) Pico.zram[a & 0x1fff] = (u8)(d >> 8);
      else elprintf(EL_ANOMALY,
                       "68k z80 write with no bus or reset! [%06x] %04x @ %06x", a, d & 0xffff, SekPc);
      return;
   }
   if (a == 0xa11200)
   {
   ...
   if (a >= SRam.start && a <= SRam.end)
   {
      ...
      if ((Pico.m.sram_reg & 0x16) ==
            0x10) // detected, not EEPROM, write not disabled
      {
