...
INLINE UINT32 update_eg_phase(FM_SLOT* SLOT, UINT32 eg_cnt)
{
   ...
   SLOT->volume = volume;
   return SLOT->tl + ((UINT32)
                      volume); /* tl is 7bit<<3, volume 0-1023 (0-2039 total) */
}
#endif
...
static void chan_render_loop(chan_rend_context* ct, int* buffer, int length)
{
   ...
   for (scounter = 0; scounter < length; scounter++)
   {
      ...
      if (ct->pack &
            8)   /* LFO enabled ? (test Earthworm Jim in between demo 1 and 2) */
      {
         ct->pack = (ct->pack & 0xffff) | (advance_lfo(ct->pack >> 16, ct->lfo_cnt,
                                           ct->lfo_cnt + ct->lfo_inc) << 16);
      ...
      while (ct->eg_timer >= EG_TIMER_OVERFLOW)
      {
         ...
         if (ct->CH->SLOT[SLOT1].state != EG_OFF) ct->vol_out1 = update_eg_phase(
                     &ct->CH->SLOT[SLOT1], ct->eg_cnt);
         if (ct->CH->SLOT[SLOT2].state != EG_OFF) ct->vol_out2 = update_eg_phase(
                     &ct->CH->SLOT[SLOT2], ct->eg_cnt);
         if (ct->CH->SLOT[SLOT3].state != EG_OFF) ct->vol_out3 = update_eg_phase(
                     &ct->CH->SLOT[SLOT3], ct->eg_cnt);
         if (ct->CH->SLOT[SLOT4].state != EG_OFF) ct->vol_out4 = update_eg_phase(
                     &ct->CH->SLOT[SLOT4], ct->eg_cnt);
      }
      ...
      eg_out = ct->vol_out1;
      if ((ct->pack & 8)
            && (ct->pack & (1 << (SLOT1 + 8)))) eg_out += ct->pack >> (((
                        ct->pack & 0xc0) >> 6) + 24);
      ...
      if (eg_out < ENV_QUIET)  /* SLOT 1 */
      {
         ...
         if (ct->pack & 0xf000) out = ((ct->op1_out >> 16) + ((ct->op1_out << 16) >> 16))
                                         << ((ct->pack & 0xf000) >> 12); /* op1_out0 + op1_out1 */
         ct->op1_out <<= 16;
...
#else
void chan_render_loop(chan_rend_context* ct, int* buffer,
...
static int chan_render(int* buffer, int length, int c,
                       UINT32 flags) // flags: stereo, ?, disabled, ?, pan_r, pan_l
{
   crct.CH = &ym2612.CH[c];
   ...
   if (crct.CH->pms)
   {
      ...
      INT32  lfo_fn_table_index_offset = lfo_pm_table[ fnum_lfo + crct.CH->pms + ((
                                            crct.pack >> 16) & 0xff) ];
      ...
      if (lfo_fn_table_index_offset)   /* LFO phase modulation active */
      {
         ...
         crct.incr1 = ((fc + crct.CH->SLOT[SLOT1].DT[kc]) * crct.CH->SLOT[SLOT1].mul) >>
                      1;
         crct.incr2 = ((fc + crct.CH->SLOT[SLOT2].DT[kc]) * crct.CH->SLOT[SLOT2].mul) >>
                      1;
         crct.incr3 = ((fc + crct.CH->SLOT[SLOT3].DT[kc]) * crct.CH->SLOT[SLOT3].mul) >>
                      1;
         crct.incr4 = ((fc + crct.CH->SLOT[SLOT4].DT[kc]) * crct.CH->SLOT[SLOT4].mul) >>
                      1;
      }
   ...
   if (crct.CH->SLOT[SLOT1].state | crct.CH->SLOT[SLOT2].state |
         crct.CH->SLOT[SLOT3].state | crct.CH->SLOT[SLOT4].state)
   {
...
INLINE void refresh_fc_eg_chan_sl3(void)
{
   ...
   {
      refresh_fc_eg_slot(&ym2612.CH[2].SLOT[SLOT1], ym2612.OPN.SL3.fc[1],
                         ym2612.OPN.SL3.kcode[1]);
      refresh_fc_eg_slot(&ym2612.CH[2].SLOT[SLOT2], ym2612.OPN.SL3.fc[2],
                         ym2612.OPN.SL3.kcode[2]);
      refresh_fc_eg_slot(&ym2612.CH[2].SLOT[SLOT3], ym2612.OPN.SL3.fc[0],
                         ym2612.OPN.SL3.kcode[0]);
      refresh_fc_eg_slot(&ym2612.CH[2].SLOT[SLOT4], ym2612.CH[2].fc ,
                         ym2612.CH[2].kcode);
   }
}
...
static void init_timetables(const UINT8* dttable)
{
   ...
   for (d = 0; d <= 3; d++)
   {
      for (i = 0; i <= 31; i++)
      {
         rate = ((double)dttable[d * 32 + i]) * SIN_LEN  * ym2612.OPN.ST.freqbase  *
...
static void OPNSetPres(int pres)
{
   ...
   /* frequency base */
   ym2612.OPN.ST.freqbase = (ym2612.OPN.ST.rate) ? ((double)ym2612.OPN.ST.clock /
                            ym2612.OPN.ST.rate) / pres : 0;
   ...
   for (i = 0; i < 4096; i++)
   {
      ...
      fn_table[i] = (UINT32)((double)i * 32 * ym2612.OPN.ST.freqbase * (1 <<
                             (FREQ_SH -
   ...
   for (i = 0; i < 8; i++)
   {
      ...
      ym2612.OPN.lfo_freq[i] = (1.0 / lfo_samples_per_step[i]) *
                               (1 << LFO_SH) * ym2612.OPN.ST.freqbase;
   }
}
...
int YM2612UpdateOne_(int* buffer, int length, int stereo, int is_buf_empty)
{
   ...
   */
   ...
   chan_render_prep();
   if (ym2612.slot_mask & 0x00000f) active_chs |= chan_render(buffer, length, 0,
            stereo | ((pan & 0x003) << 4)) << 0;
   if (ym2612.slot_mask & 0x0000f0) active_chs |= chan_render(buffer, length, 1,
            stereo | ((pan & 0x00c) << 2)) << 1;
   if (ym2612.slot_mask & 0x000f00) active_chs |= chan_render(buffer, length, 2,
            stereo | ((pan & 0x030))) << 2;
   if (ym2612.slot_mask & 0x00f000) active_chs |= chan_render(buffer, length, 3,
            stereo | ((pan & 0x0c0) >> 2)) << 3;
   if (ym2612.slot_mask & 0x0f0000) active_chs |= chan_render(buffer, length, 4,
            stereo | ((pan & 0x300) >> 4)) << 4;
   if (ym2612.slot_mask & 0xf00000) active_chs |= chan_render(buffer, length, 5,
            stereo | ((pan & 0xc00) >> 6) | (ym2612.dacen << 2)) << 5;
   chan_render_finish();
   ...
   case 1:  /* data port 0    */
      ...
      case 0x20:  /* 0x20-0x2f Mode */
         ...
         case 0x28:  /* key on / off */
         {
            ...
            if (v & 0x04) c += 3;
            if (v & 0x10) FM_KEYON(c, SLOT1);
            else FM_KEYOFF(c, SLOT1);
            if (v & 0x20) FM_KEYON(c, SLOT2);
            else FM_KEYOFF(c, SLOT2);
            if (v & 0x40) FM_KEYON(c, SLOT3);
            else FM_KEYOFF(c, SLOT3);
            if (v & 0x80) FM_KEYON(c, SLOT4);
            else FM_KEYOFF(c, SLOT4);
            break;
         }
         case 0x2a:  /* DAC data (YM2612) */
            ym2612.dacout = ((int)v - 0x80) <<
                            6;  /* level unknown (notaz: 8 seems to be too much) */
            ret = 0;
            break;
...
int YM2612PicoTick_(int n)
{
   ...
   if (ym2612.OPN.ST.mode & 0x01
         && (ym2612.OPN.ST.TAT += 64 * n) >= ym2612.OPN.ST.TAC)
   {
   ...
   if (ym2612.OPN.ST.mode & 0x02
         && (ym2612.OPN.ST.TBT += 64 * n) >= ym2612.OPN.ST.TBC)
   {
...
void YM2612PicoStateSave2(int tat, int tbt)
{
   ...
   for (c = 0; c < 3; c++)
   {
      for (s = 0; s < 4; s++)
      {
         ss.state_phase = (ym2612.CH[c].SLOT[s].state << 29) |
                          (ym2612.CH[c].SLOT[s].phase >> 3);
