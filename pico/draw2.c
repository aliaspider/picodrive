...
unsigned short* PicoCramHigh =
   Pico.cram; // pointer to CRAM buff (0x40 shorts), converted to native device color (works only with 16bit for now)
void (*PicoPrepareCram)() =
   0;          // prepares PicoCramHigh for renderer to use
...
static int TileXnormYnorm(unsigned char* pd, int addr, unsigned char pal)
{
   ...
   for (i = 8; i; i--, addr += 2, pd += LINE_WIDTH)
   {
      ...
      t = pack & 0x0000f000;
      if (t) pd[0] = (unsigned char)((t >> 12) | pal);
      t = pack & 0x00000f00;
      if (t) pd[1] = (unsigned char)((t >> 8) | pal);
      t = pack & 0x000000f0;
      if (t) pd[2] = (unsigned char)((t >> 4) | pal);
      t = pack & 0x0000000f;
      if (t) pd[3] = (unsigned char)((t) | pal);
      t = pack & 0xf0000000;
      if (t) pd[4] = (unsigned char)((t >> 28) | pal);
      t = pack & 0x0f000000;
      if (t) pd[5] = (unsigned char)((t >> 24) | pal);
      t = pack & 0x00f00000;
      if (t) pd[6] = (unsigned char)((t >> 20) | pal);
      t = pack & 0x000f0000;
      if (t) pd[7] = (unsigned char)((t >> 16) | pal);
      blank = 0;
   }
...
static int TileXflipYnorm(unsigned char* pd, int addr, unsigned char pal)
{
   ...
   for (i = 8; i; i--, addr += 2, pd += LINE_WIDTH)
   {
      ...
      t = pack & 0x000f0000;
      if (t) pd[0] = (unsigned char)((t >> 16) | pal);
      t = pack & 0x00f00000;
      if (t) pd[1] = (unsigned char)((t >> 20) | pal);
      t = pack & 0x0f000000;
      if (t) pd[2] = (unsigned char)((t >> 24) | pal);
      t = pack & 0xf0000000;
      if (t) pd[3] = (unsigned char)((t >> 28) | pal);
      t = pack & 0x0000000f;
      if (t) pd[4] = (unsigned char)((t) | pal);
      t = pack & 0x000000f0;
      if (t) pd[5] = (unsigned char)((t >> 4) | pal);
      t = pack & 0x00000f00;
      if (t) pd[6] = (unsigned char)((t >> 8) | pal);
      t = pack & 0x0000f000;
      if (t) pd[7] = (unsigned char)((t >> 12) | pal);
      blank = 0;
   }
   return blank; // Tile blank?
}
...
static void DrawWindowFull(int start, int end, int prio)
{
   ...
   if ((code >> 15) != prio)
      return; // hack: just assume that whole window uses same priority

   scrpos += 8 * LINE_WIDTH + 8;
   scrpos += 8 * LINE_WIDTH * (start - START_ROW);

   // do a window until we reach planestart row
   for (trow = start; trow < end;
         trow++, nametab += nametab_step) // current tile row
   {
      for (tilex = tile_start; tilex < tile_end; tilex++)
      {
         ...
         {
         case 0:
            zero = TileXnormYnorm(scrpos + (tilex << 3), addr, pal);
            break;
         case 1:
            zero = TileXflipYnorm(scrpos + (tilex << 3), addr, pal);
            break;
         case 2:
            zero = TileXnormYflip(scrpos + (tilex << 3), addr, pal);
            break;
         case 3:
            zero = TileXflipYflip(scrpos + (tilex << 3), addr, pal);
            break;
         }
...
static void DrawLayerFull(int plane, int* hcache, int planestart, int planeend)
{
   ...
   if (vscroll & 7)
      planeend++; // we have vertically clipped tiles due to vscroll, so we need 1 more row
   ...
   *hcache++ = 8 - (vscroll & 7); // push y-offset to tilecache
   ...
   for (trow = planestart; trow < planeend; trow++)  // current tile row
   {
      ...
      nametab_row = nametab + (((trow + (vscroll >> 3))&ymask) <<
                               shift[width]); // pointer to nametable entries for this row
      ...
      for (; cellc; dx += 8, tilex++, cellc--)
      {
         ...
         {
         case 0:
            zero = TileXnormYnorm(scrpos + dx, addr, pal);
            break;
         case 1:
            zero = TileXflipYnorm(scrpos + dx, addr, pal);
            break;
         case 2:
            zero = TileXnormYflip(scrpos + dx, addr, pal);
            break;
         case 3:
            zero = TileXflipYflip(scrpos + dx, addr, pal);
            break;
         }
         if (zero) blank = code; // We know this tile is blank now
      }
...
static void DrawTilesFromCacheF(int* hc)
{
   ...
   while ((code = *hc++))
   {
      ...
      {
      case 0:
         zero = TileXnormYnorm(pd + ((code >> 16) & 0x1ff), addr, pal);
         break;
      case 1:
         zero = TileXflipYnorm(pd + ((code >> 16) & 0x1ff), addr, pal);
         break;
      case 2:
         zero = TileXnormYflip(pd + ((code >> 16) & 0x1ff), addr, pal);
         break;
      case 3:
         zero = TileXflipYflip(pd + ((code >> 16) & 0x1ff), addr, pal);
         break;
      }
...
static void DrawSpriteFull(unsigned int* sprite)
{
   ...
   if (code & 0x0800)
   {
      tdeltax = -tdeltax;   // Flip X
      tile += height * (width - 1);
   }
   if (code & 0x1000)
   {
      tdeltay = -tdeltay;   // Flip Y
      tile += height - 1;
   }
   ...
   for (; height > 0; height--, sy += 8, tile += tdeltay)
   {
      ...
      for (; w; w--, x += 8, t += tdeltax)
      {
         ...
         {
         case 0:
            TileXnormYnorm(scrpos + x, t << 4, pal);
            break;
         case 1:
            TileXflipYnorm(scrpos + x, t << 4, pal);
            break;
         case 2:
            TileXnormYflip(scrpos + x, t << 4, pal);
            break;
         case 3:
            TileXflipYflip(scrpos + x, t << 4, pal);
            break;
         }
      }
...
static void DrawAllSpritesFull(int prio, int maxwidth)
{
   ...
   for (i = u = 0; u < 80; u++)
   {
      ...
      sprite = (unsigned int*)(Pico.vram + ((table + (link << 2)) &
                                            0x7ffc)); // Find sprite
      ...
      if (!sx)
      {
         ...
         if (maskrange)
         {
            ...
            if ((maskrange >> 16) + 1 >= sy && (maskrange >> 16) <= to
                  && (maskrange & 0xffff) < sy) sy = (maskrange & 0xffff);
            else if ((maskrange & 0xffff) - 1 <= to && (maskrange & 0xffff) >= sy
                     && (maskrange >> 16) > to) to = (maskrange >> 16);
         }
...
static void BackFillFull(int reg7)
{
   ...
   memset32((int*)PicoDraw2FB, back,
            LINE_WIDTH * (8 + (END_ROW - START_ROW) * 8) / 4);
}
#endif
...
static void DrawDisplayFull(void)
{
   ...
   int planestart = START_ROW,
       planeend = END_ROW; // plane A start/end when window shares display with plane A (in tile rows or columns)
   ...
   if (PicoDrawMask & PDRAW_LAYERA_ON) switch (hvwin)
      ...
      case 3:
         ...
         DrawWindowFull(winstart & ~0xff0000, (winend & ~0xff0000) | (maxcolc << 16),
                        0); // h
         DrawWindowFull((winstart & ~0xff) | START_ROW, (winend & ~0xff) | END_ROW,
                        0);    // v
         break;
