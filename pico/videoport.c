...
int (*PicoDmaHook)(unsigned int source, int len, unsigned short** srcp,
                   unsigned short** limitp) = NULL;
...
static void VideoWrite(u16 d)
{
   ...
   case 1:
      if (a & 1) d = (u16)((d << 8) | (d >>
                                          8)); // If address is odd, bytes are swapped (which game needs this?)
   ...
   case 3:
      Pico.m.dirtyPal = 1;
      Pico.cram [(a >> 1) & 0x003f] = d;
      break; // wraps (Desert Strike)
   case 5:
      Pico.vsram[(a >> 1) & 0x003f] = d;
      break;
...
static unsigned int VideoRead(void)
{
   ...
   {
   case 0:
      d = Pico.vram [a & 0x7fff];
      break;
   case 8:
      d = Pico.cram [a & 0x003f];
      break;
   case 4:
      d = Pico.vsram[a & 0x003f];
      break;
   default:
      elprintf(EL_ANOMALY, "VDP read with bad type %i", Pico.video.type);
      break;
   }
...
static void DmaSlow(int len)
{
   ...
   elprintf(EL_VDPDMA, "DmaSlow[%i] %06x->%04x len %i inc=%i blank %i [%i] @ %06x",
            Pico.video.type, source, a, len, inc, (Pico.video.status & 8)
            || !(Pico.video.reg[1] & 0x40),
            SekCyclesDone(), SekPc);

   Pico.m.dma_xfers += len;
   if ((PicoAHW & PAHW_MCD)
         && (PicoOpt & POPT_EN_MCD_PSYNC)) SekCyclesBurn(CheckDMA());
   ...
   else if (PicoAHW & PAHW_MCD)
   {
      ...
      {
         elprintf(EL_VDPDMA | EL_ANOMALY,
                  "DmaSlow[%i] %06x->%04x: FIXME: unsupported src", Pico.video.type, source, a);
         return;
      }
   }
   else
   {
      ...
      {
         elprintf(EL_VDPDMA | EL_ANOMALY, "DmaSlow[%i] %06x->%04x: invalid src",
                  Pico.video.type, source, a);
         return;
      }
   }
...
static void DmaFill(int data)
{
   ...
   Pico.video.addr = a;
   // update length
   Pico.video.reg[0x13] = Pico.video.reg[0x14] =
                             0; // Dino Dini's Soccer (E) (by Haze)
...
PICO_INTERNAL_ASM void PicoVideoWrite(unsigned int a, unsigned short d)
{
   ...
   if (a == 0x00) // Data port 0 or 2
   {
      ...
      if (Pico.m.scanline < 224 && (pvid->reg[1] & 0x40) &&
            !(!pvid->pending &&
              ((pvid->command & 0xc00000f0) == 0x40000010
               && Pico.vsram[pvid->addr >> 1] == d))
         )
         DrawSync(0);
      ...
      if ((pvid->command & 0x80) && (pvid->reg[1] & 0x10)
            && (pvid->reg[0x17] >> 6) == 2)
         DmaFill(d);
      else
      {
         // preliminary FIFO emulation for Chaos Engine, The (E)
         if (!(pvid->status & 8) && (pvid->reg[1] & 0x40)
               && !(PicoOpt & POPT_DIS_VDP_FIFO)) // active display?
         {
            ...
            elprintf(EL_ASVDP, "VDP data write: %04x [%06x] {%i} #%i @ %06x", d,
                     Pico.video.addr,
   ...
   if (a == 0x04) // Control (command) port 4 or 6
   {
      ...
      else
      {
         if ((d & 0xc000) == 0x8000)
         {
            ...
            int blank_on = 0;
            pvid->type =
               0; // register writes clear command (else no Sega logo in Golden Axe II)
            if (num > 0x0a && !(pvid->reg[1] & 4))
            {
               elprintf(EL_ANOMALY, "%02x written to reg %02x in SMS mode @ %06x", d, num,
                        SekPc);
               return;
            }
...
PICO_INTERNAL_ASM unsigned int PicoVideoRead(unsigned int a)
{
   ...
   if ((a & 0x1c) == 0x08)
   {
      ...
      elprintf(EL_HVCNT, "hv: %02x %02x (%i) @ %06x", d, Pico.video.v_counter,
               SekCyclesDone(), SekPc);
...
unsigned int PicoVideoRead8(unsigned int a)
{
   ...
   case 5:
      ...
      //if (PicoOpt&POPT_ALT_RENDERER) d|=0x0020; // sprite collision (Shadow of the Beast)
      d |= ((Pico.video.reg[1] & 0x40) ^ 0x40) >>
           3; // set V-Blank if display is disabled
      ...
      return d;
   case 8: // hv counter
      elprintf(EL_HVCNT, "vcounter: %02x (%i) @ %06x", Pico.video.v_counter,
               SekCyclesDone(), SekPc);
