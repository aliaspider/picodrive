...
PICO_INTERNAL u32 PicoCheckPc(u32 pc)
{
   ...
   if ((pc << 8) == 0)
   {
      ...
      return (int)Pico.rom +
             Pico.romsize; // common crash condition, may happen with bad ROMs
   }
...
static u32 OtherRead16End(u32 a, int realsize)
{
   ...
   */
   ...
   {
      if (*(int*)(Pico.rom + 0x123e4) == 0x00550c39
            && *(int*)(Pico.rom + 0x123e8) == 0x00000040)   // Super Bubble Bobble (Unl) [!]
      {
         if (a == 0x400000)
         {
            d = 0x55 << 8;
      ...
      else if (*(int*)(Pico.rom + 0x008c4) == 0x66240055
               && *(int*)(Pico.rom + 0x008c8) == 0x00404df9)   // Smart Mouse (Unl)
      {
         if (a == 0x400000)
         {
            d = 0x55 << 8;
      ...
      else if (*(int*)(Pico.rom + 0x00404) == 0x00a90600
               && *(int*)(Pico.rom + 0x00408) ==
               0x6708b013)   // King of Fighters '98, The (Unl) [!]
      {
         if (a == 0x480000 || a == 0x4800e0 || a == 0x4824a0 || a == 0x488880)
         {
            d = 0xaa << 8;
            goto end;
         }
      ...
      else if (*(int*)(Pico.rom + 0x01b24) == 0x004013f9
               && *(int*)(Pico.rom + 0x01b28) == 0x00ff0000)   // Mahjong Lover (Unl) [!]
      {
         if (a == 0x400000)
         {
            d = 0x90 << 8;
         ...
         {
            d = 0xd3 << 8;
            // this one doesn't seem to be needed, the code does 2 comparisons and only then
            goto end;
         }
         // checks the result, which is of the above one. Left it just in case.
      }
      else if (*(int*)(Pico.rom + 0x05254) == 0x0c3962d0
               && *(int*)(Pico.rom + 0x05258) == 0x00400055)   // Elf Wor (Unl)
      {
         if (a == 0x400000)
         {
            d = 0x55 << 8;
         ...
         {
            d = 0xc9 << 8;   // this check is done if the above one fails
            goto end;
         }
         else if (a == 0x400002)
         {
   ...
   {
      if (*(int*)(Pico.rom + 0xc8af0) == 0x30133013
            && *(int*)(Pico.rom + 0xc8af4) == 0x000f0240)   // Rockman X3 (Unl) [!]
      {
         d = 0x0c;
         goto end;
      }
      else if (*(int*)(Pico.rom + 0x28888) == 0x07fc0000
               && *(int*)(Pico.rom + 0x2888c) == 0x4eb94e75)   // Bug's Life, A (Unl) [!]
      {
         d = 0x28;
         goto end; // does the check from RAM
      }
      else if (*(int*)(Pico.rom + 0xc8778) == 0x30133013
               && *(int*)(Pico.rom + 0xc877c) == 0x000f0240)   // Super Mario Bros. (Unl) [!]
      {
         d = 0x0c;
         goto end; // seems to be the same code as in Rockman X3 (Unl) [!]
      }
      else if (*(int*)(Pico.rom + 0xf20ec) == 0x30143013
               && *(int*)(Pico.rom + 0xf20f0) == 0x000f0200)   // Super Mario 2 1998 (Unl) [!]
      {
         d = 0x0a;
         goto end;
      }
   ...
   elprintf(EL_UIO, "strange r%i: [%06x] %04x @%06x", realsize, a & 0xffffff, d,
            SekPc);
   return d;
}
...
#endif
   elprintf(EL_UIO, "strange w%i: %06x, %08x @%06x", realsize, a & 0xffffff, d,
            SekPc);
...
PICO_INTERNAL_ASM u32 PicoRead8(u32 a)
{
   ...
   log_io(a, 8, 0);
   if ((a & 0xff4000) == 0xa00000)
   {
      d = z80Read8(a);   // Z80 Ram
      goto end;
   }

   if ((a & 0xe700e0) == 0xc00000)
   {
      d = PicoVideoRead8(a);   // VDP
      goto end;
   }
...
PICO_INTERNAL void PicoMemSetup(void)
{
   ...
   // now real ROM
   for (i = 0; i < M68K_FETCHBANK1
         && (i << (24 - FAMEC_FETCHBITS)) < Pico.romsize; i++)
...
void (*pm68k_write_memory_16)(unsigned int address,
                              unsigned short value) = NULL;
void (*pm68k_write_memory_32)(unsigned int address,
...
static unsigned int  m68k_read_8(unsigned int a, int do_fake)
{
   a &= 0xffffff;
   if (a < Pico.romsize
         && m68ki_cpu_p == &PicoCpuMM68k)    return *(u8*)(Pico.rom + (a ^ 1)); // Rom
...
static unsigned int  m68k_read_16(unsigned int a, int do_fake)
{
   a &= 0xffffff;
   if (a < Pico.romsize
         && m68ki_cpu_p == &PicoCpuMM68k)    return *(u16*)(Pico.rom + (a & ~1)); // Rom
...
{
   if ((a & 0xe00000) == 0xe00000) return *(u8*)(Pico.ram + ((
            a ^ 1) & 0xffff)); // Ram
   return 0;
}
...
{
   return pm68k_read_memory_8(address) & 0xff;
}
unsigned int m68k_read_memory_16(unsigned int address)
{
   return pm68k_read_memory_16(address) & 0xffff;
}
unsigned int m68k_read_memory_32(unsigned int address)
{
   return pm68k_read_memory_32(address);
}
void m68k_write_memory_8(unsigned int address, unsigned int value)
{
   pm68k_write_memory_8(address, (u8)value);
}
void m68k_write_memory_16(unsigned int address, unsigned int value)
{
   pm68k_write_memory_16(address, (u16)value);
}
void m68k_write_memory_32(unsigned int address, unsigned int value)
{
   pm68k_write_memory_32(address, value);
}
...
void ym2612_sync_timers(int z80_cycles, int mode_old, int mode_new)
{
   ...
   if (mode_new & 1)
      elprintf(EL_YMTIMER, "timer a upd to %i @ %i", timer_a_next_oflow >> 8,
               z80_cycles);
   ...
   if (mode_new & 2)
      elprintf(EL_YMTIMER, "timer b upd to %i @ %i", timer_b_next_oflow >> 8,
               z80_cycles);
}
      ...
      case 0x25:   // timer A Low 2
      {
         ...
         if (ym2612.OPN.ST.TA != TAnew)
         {
            ...
            if (ym2612.OPN.ST.mode & 1)
            {
               ...
               int cycles = is_from_z80 ? z80_cyclesDone() : cycles_68k_to_z80(
                               SekCyclesDone());
               timer_a_next_oflow = (cycles << 8) + timer_a_step;
            }
            elprintf(EL_YMTIMER, "timer a set to %i, %i", 1024 - TAnew,
                     timer_a_next_oflow >> 8);
         }
         return 0;
      }
      case 0x26: // timer B
...
u32 ym2612_read_local_z80(void)
{
   ...
   elprintf(EL_YMTIMER, "timer z80 read %i, sched %i, %i @ %i|%i",
            ...
            timer_a_next_oflow >> 8, timer_b_next_oflow >> 8, xcycles >> 8,
            (xcycles >> 8) / 228);
...
void ym2612_pack_state(void)
{
   ...
   if (timer_a_next_oflow != TIMER_NO_OFLOW)
      tat = (int)((double)(timer_a_step - timer_a_next_oflow) /
                  (double)timer_a_step * tac * 65536);
   if (timer_b_next_oflow != TIMER_NO_OFLOW)
      tbt = (int)((double)(timer_b_step - timer_b_next_oflow) /
                  (double)timer_b_step * tbc * 65536);
...
void ym2612_unpack_state(void)
{
   ...
   if (ym2612.OPN.ST.mode & 2)
      ...
      timer_b_next_oflow = TIMER_NO_OFLOW;
   elprintf(EL_YMTIMER, "load: %i/%i, timer_a_next_oflow %i", tat >> 16, tac >> 16,
            timer_a_next_oflow >> 8);
   elprintf(EL_YMTIMER, "load: %i/%i, timer_b_next_oflow %i", tbt >> 16, tbc >> 16,
            timer_b_next_oflow >> 8);
}
