...
PICO_INTERNAL int Get_Status_CDD_c0(void)
{
   ...
   if ((Pico_mcd->cdd.Status & 0x0F00) == 0x0200)
      Pico_mcd->cdd.Status = (Pico_mcd->scd.Status_CDD & 0xFF00) |
                             (Pico_mcd->cdd.Status & 0x00FF);
...
PICO_INTERNAL int Get_Pos_CDD_c20(void)
{
   ...
   cdprintf("Status CDD = %.4X  Status = %.4X", Pico_mcd->scd.Status_CDD,
            Pico_mcd->cdd.Status);
...
PICO_INTERNAL int Get_Track_Pos_CDD_c21(void)
{
   ...
   elapsed_time = Pico_mcd->scd.Cur_LBA - Track_to_LBA(LBA_to_Track(
...
PICO_INTERNAL int Get_Total_Lenght_CDD_c23(void)
{
   ...
   Pico_mcd->cdd.Minute = INT_TO_BCDW(
                             Pico_mcd->TOC.Tracks[Pico_mcd->TOC.Last_Track].MSF.M);
   Pico_mcd->cdd.Seconde = INT_TO_BCDW(
                              Pico_mcd->TOC.Tracks[Pico_mcd->TOC.Last_Track].MSF.S);
   Pico_mcd->cdd.Frame = INT_TO_BCDW(
                            Pico_mcd->TOC.Tracks[Pico_mcd->TOC.Last_Track].MSF.F);
   Pico_mcd->cdd.Ext = 0;
...
PICO_INTERNAL int Get_Track_Adr_CDD_c25(void)
{
   ...
   track_number = (Pico_mcd->s68k_regs[0x38 + 10 + 4] & 0xF) * 10 +
                  (Pico_mcd->s68k_regs[0x38 + 10 + 5] & 0xF);
   ...
   if (track_number > Pico_mcd->TOC.Last_Track) track_number =
         Pico_mcd->TOC.Last_Track;
   else if (track_number < 1) track_number = 1;

   Pico_mcd->cdd.Minute = INT_TO_BCDW(Pico_mcd->TOC.Tracks[track_number -
                                      1].MSF.M);
   Pico_mcd->cdd.Seconde = INT_TO_BCDW(Pico_mcd->TOC.Tracks[track_number -
                                       1].MSF.S);
...
PICO_INTERNAL int Play_CDD_c3(void)
{
   ...
   MSF.M = (Pico_mcd->s68k_regs[0x38 + 10 + 2] & 0xF) * 10 +
           (Pico_mcd->s68k_regs[0x38 + 10 + 3] & 0xF);
   MSF.S = (Pico_mcd->s68k_regs[0x38 + 10 + 4] & 0xF) * 10 +
           (Pico_mcd->s68k_regs[0x38 + 10 + 5] & 0xF);
   MSF.F = (Pico_mcd->s68k_regs[0x38 + 10 + 6] & 0xF) * 10 +
           (Pico_mcd->s68k_regs[0x38 + 10 + 7] & 0xF);
   ...
   cdprintf("Read : Cur LBA = %d, M=%d, S=%d, F=%d", Pico_mcd->scd.Cur_LBA, MSF.M,
            MSF.S, MSF.F);
...
PICO_INTERNAL int Pause_CDD_c6(void)
{
   ...
   Pico_mcd->scd.Status_CDC &=
      ~1;        // Stop CDC read to start a new one if raw data
