...
static void gfx_cd_start(void)
{
   ...
   _rot_comp.Function = (_rot_comp.Reg_58 & 7) | (Pico_mcd->s68k_regs[3] &
                        0x18);   // Jmp_Adr
...
static void gfx_do(unsigned int func, unsigned short* stamp_base,
                   unsigned int H_Dot)
{
...
PICO_INTERNAL void gfx_cd_update(void)
{
   ...
   if (PicoOpt & POPT_EN_MCD_GFX)
   {
      ...
      unsigned short* stamp_base = (unsigned short*)(Pico_mcd->word_ram2M +
                                   _rot_comp.Stamp_Map_Adr);
...
PICO_INTERNAL_ASM unsigned int gfx_cd_read(unsigned int a)
{
   ...
   case 0x66:
      break;
   default:
      dprintf("gfx_cd_read FIXME: unexpected address: %02x", a);
      break;
   }
...
PICO_INTERNAL_ASM void gfx_cd_write16(unsigned int a, unsigned int d)
{
   ...
   default:
      dprintf("gfx_cd_write16 FIXME: unexpected address: %02x", a);
      return;
   }
}
...
PICO_INTERNAL void DmaSlowCell(unsigned int source, unsigned int a, int len,
                               unsigned char inc)
{
   unsigned char* base;
