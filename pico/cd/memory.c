...
static u32 m68k_reg_read16(u32 a)
{
   ...
   case 0:
      d = ((Pico_mcd->s68k_regs[0x33] << 13) & 0x8000) |
          Pico_mcd->m.busreq; // here IFL2 is always 0, just like in Gens
      goto end;
   case 2:
...
void m68k_reg_write8(u32 a, u32 d)
{
   ...
   case 0:
      d &= 1;
      if ((d & 1) && (Pico_mcd->s68k_regs[0x33] & (1 << 2)))
      {
   ...
   case 1:
      d &= 3;
      if (!(d & 1)) Pico_mcd->m.state_flags |=
            1; // reset pending, needed to be sure we fetch the right vectors on reset
      if ((Pico_mcd->m.busreq & 1) != (d & 1)) dprintf("m68k: s68k reset %i",
               !(d & 1));
      if ((Pico_mcd->m.busreq & 2) != (d & 2)) dprintf("m68k: s68k brq %i",
               (d & 2) >> 1);
   ...
   case 3:
   {
      ...
      d &= 0xc2;
      if ((dold >> 6) != ((d >> 6) & 3))
         dprintf("m68k: prg bank: %i -> %i", (Pico_mcd->s68k_regs[a] >> 6),
                 ((d >> 6) & 3));
      ...
      else
      {
         ...
         {
            Pico_mcd->m.state_flags |=
               2; // we must delay setting DMNA bit (needed for Silpheed)
            d &= ~2;
         }
         else
...
void s68k_reg_write8(u32 a, u32 d)
{
   ...
   case 0xd:
      ...
      return;
   case 0xe:
      Pico_mcd->s68k_regs[0xf] = (d >> 1) | (d <<
                                             7); // ror8 1, Gens note: Dragons lair
      return;
   case 0x31:
   ...
   case 0x33: // IRQ mask
      ...
      if ((d & (1 << 4)) && (Pico_mcd->s68k_regs[0x37] & 4)
            && !(Pico_mcd->s68k_regs[0x33] & (1 << 4)))
         CDD_Export_Status();
      break;
   case 0x34: // fader
...
static u32 OtherRead16End(u32 a, int realsize)
{
   ...
   elprintf(EL_UIO, "m68k FIXME: unusual r%i: %06x @%06x", realsize & ~1,
            (a & 0xfffffe) + (realsize & 1), SekPc);
...
static void OtherWrite8End(u32 a, u32 d, int realsize)
{
   ...
   elprintf(EL_UIO, "m68k FIXME: strange w%i: [%06x], %08x @%06x", realsize,
            a & 0xffffff, d, SekPc);
}
...
static u32 PicoReadM68k16(u32 a)
{
   ...
   case 0x02>>1: // prg RAM
      ...
      {
         u8* prg_bank = Pico_mcd->prg_ram_b[Pico_mcd->s68k_regs[3] >> 6];
         wrdprintf("m68k_prgram r16: [%i,%06x] @%06x", Pico_mcd->s68k_regs[3] >> 6, a,
                   SekPc);
...
static void PicoWriteM68k16(u32 a, u16 d)
{
   ...
   {
      u8* prg_bank = Pico_mcd->prg_ram_b[Pico_mcd->s68k_regs[3] >> 6];
      wrdprintf("m68k_prgram w16: [%i,%06x] %04x @%06x", Pico_mcd->s68k_regs[3] >> 6,
                a, d, SekPc);
...
static u32 PicoReadS68k8(u32 a)
{
   ...
   elprintf(EL_UIO, "s68k r8 : %06x,   %02x @%06x", a & 0xffffff, (u8)d,
            SekPcS68k);
...
static u32 PicoReadS68k32(u32 a)
{
   ...
   if ((a & 0xff0000) == 0xfe0000)
   {
      ...
      a = (a >> 1) & 0x1fff;
      d = Pico_mcd->bram[a++] <<
          16;     // middle endian? TODO: verify against Fusion..
...
static __inline int PicoMemBaseS68k(u32 pc)
{
   ...
   if ((pc & 0xfc0000) ==
         0x080000)      // WORD RAM 2M area (assume we are in the right mode..)
...
void PicoMemResetCD(int r3)
{
   ...
   if (!(r3 & 4))
   {
      for (i = M68K_FETCHBANK1 * 2 / 16; (i << (24 - FAMEC_FETCHBITS)) < 0x240000;
            i++)
         PicoCpuFM68k.Fetch[i] = (unsigned int)Pico_mcd->word_ram2M - 0x200000;
   }
   else
   {
      for (i = M68K_FETCHBANK1 * 2 / 16; (i << (24 - FAMEC_FETCHBITS)) < 0x220000;
            i++)
         PicoCpuFM68k.Fetch[i] = (unsigned int)Pico_mcd->word_ram1M[r3 & 1] - 0x200000;
      for (i = M68K_FETCHBANK1 * 0x0c / 0x100;
            (i << (24 - FAMEC_FETCHBITS)) < 0x0e0000; i++)
         PicoCpuFS68k.Fetch[i] = (unsigned int)Pico_mcd->word_ram1M[(r3 & 1) ^ 1] -
                                 0x0c0000;
   }
}
#endif
...
PICO_INTERNAL void PicoMemSetupCD(void)
{
   ...
   // setup FAME fetchmap
   {
      ...
      // PRG RAM is default
      for (i = 0; i < M68K_FETCHBANK1; i++)
         PicoCpuFS68k.Fetch[i] = (unsigned int)Pico_mcd->prg_ram - (i <<
                                 (24 - FAMEC_FETCHBITS));
      // real PRG RAM
      for (i = 0; i < M68K_FETCHBANK1 && (i << (24 - FAMEC_FETCHBITS)) < 0x80000; i++)
         PicoCpuFS68k.Fetch[i] = (unsigned int)Pico_mcd->prg_ram;
      // WORD RAM 2M area
      for (i = M68K_FETCHBANK1 * 0x08 / 0x100; i < M68K_FETCHBANK1
            && (i << (24 - FAMEC_FETCHBITS)) < 0xc0000; i++)
...
{
   if (m68ki_cpu_p == &PicoCpuMS68k) PicoWriteS68k8(a, d);
   else PicoWriteM68k8(a, d);
}
static void PicoWriteCD16w(unsigned int a, unsigned short d)
{
   if (m68ki_cpu_p == &PicoCpuMS68k) PicoWriteS68k16(a, d);
   else PicoWriteM68k16(a, d);
}
static void PicoWriteCD32w(unsigned int a, unsigned int d)
{
   if (m68ki_cpu_p == &PicoCpuMS68k) PicoWriteS68k32(a, d);
   else PicoWriteM68k32(a, d);
}
...
static unsigned int  m68k_read_pcrelative_CD8(unsigned int a)
{
   ...
   if (m68ki_cpu_p == &PicoCpuMS68k)
   {
      ...
      if ((a & 0xfc0000) == 0x080000
            && !(Pico_mcd->s68k_regs[3] & 4)) // word RAM (2M area: 080000-0bffff)
         return *(u8*)(Pico_mcd->word_ram2M + ((a ^ 1) & 0x3ffff));
      if ((a & 0xfe0000) == 0x0c0000
            && (Pico_mcd->s68k_regs[3] & 4)) // word RAM (1M area: 0c0000-0dffff)
      {
         int bank = (Pico_mcd->s68k_regs[3] & 1) ^ 1;
...
static unsigned int  m68k_read_pcrelative_CD32(unsigned int a)
{
   ...
   if (m68ki_cpu_p == &PicoCpuMS68k)
   {
      if (a < 0x80000)
      {
         u16* pm = (u16*)(Pico_mcd->prg_ram + (a & ~1));   // PRG Ram
         return (pm[0] << 16) | pm[1];
      }
      if ((a & 0xfc0000) == 0x080000
            && !(Pico_mcd->s68k_regs[3] & 4)) // word RAM (2M area: 080000-0bffff)
      {
         pm = (u16*)(Pico_mcd->word_ram2M + (a & 0x3fffe));
         return (pm[0] << 16) | pm[1];
      }
      if ((a & 0xfe0000) == 0x0c0000
            && (Pico_mcd->s68k_regs[3] & 4)) // word RAM (1M area: 0c0000-0dffff)
...
extern void (*pm68k_write_memory_16)(unsigned int address,
                                     unsigned short value);
extern void (*pm68k_write_memory_32)(unsigned int address,
                                     unsigned int   value);
